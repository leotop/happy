...
<div id="basket_items_list">
	...
	<div class="cart-products">
		<table id="basket_items" class="cart-table">
			<thead>
				<tr>
					<?
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
						...
						<? elseif ($arHeader["id"] == "SUM"): ?>
							<th class="cart-subtotal" id="col_<?=$arHeader["id"];?>"><?=$arHeader["name"]; ?></th>
							...
							?>
								<td class="cart-qty">
									...
									<div class="input-group product-qty">
							            <a href="javascript:void(0);" class="input-group-addon product-minus" onclick="setQuantity(<?=$arItem["ID"]?>, <?=$arItem["MEASURE_RATIO"]?>, 'down', <?=$useFloatQuantityJS?>);"><i class="fa fa-minus"></i></a>
										...
										<a href="javascript:void(0);" class="input-group-addon product-plus" onclick="setQuantity(<?=$arItem["ID"]?>, <?=$arItem["MEASURE_RATIO"]?>, 'up', <?=$useFloatQuantityJS?>);"><i class="fa fa-plus"></i></a>
									...
									<? endif; ?>
									 ...
									 <? if ($bDelayColumn): ?>
							        	<div class="btn-remove"><i class="fa fa-ellipsis-h"></i> <a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?>" title="<?=GetMessage("SALE_DELAY")?>"><?=GetMessage("SALE_DELAY")?></a></div>
									<? endif; ?>
		...
		<div class="discount" id="coupons_block">
			<div class="discount-form">
				<div class="form-group">
					<label><?=GetMessage("STB_COUPON_PROMT")?></label>

					<input type="text" id="coupon" name="COUPON" class="form-control" value="" onchange="enterCoupon();">

					<a class="btn btn-default btn-apply-coupon" href="javascript:void(0)" onclick="enterCoupon();" title="<?=GetMessage('SALE_COUPON_APPLY_TITLE'); ?>"><i class="fa fa-gift"></i><?=GetMessage('SALE_COUPON_APPLY'); ?></a>
				</div>
			</div>

			<? if (!empty($arResult['COUPON_LIST']) && 3==2) {
		...
		<div class="cart-buttons">
			<div class="checkout-button">
				...
				<?endif;?>
				<a href="javascript:void(0)" onclick="checkOut();" class="btn btn-checkout"><?=GetMessage("SALE_ORDER")?></a>
			</div>
		</div>
			...
			<table class="table">
				...
				<?if ($arParams["PRICE_VAT_SHOW_VALUE"] == "Y"):?>
					<tr class="sub-totals">
						...
						<td id="allSum_wVAT_FORMATED" class="price"><?=$arResult["allSum_wVAT_FORMATED"]?></td>
					</tr>
					<?if (floatval($arResult["DISCOUNT_PRICE_ALL"]) > 0):?>
						<tr class="sub-totals">
							<td class="custom_t1"></td>
							<td class="custom_t2 price" style="text-decoration:line-through; color:#828282;" id="PRICE_WITHOUT_DISCOUNT">
					...
					if (floatval($arResult['allVATSum']) > 0):
						...
						<tr class="sub-totals">
							...
							<td id="allVATSum_FORMATED" class="price"><?=$arResult["allVATSum_FORMATED"]?></td>
						</tr>
						<?
					endif;
					?>
