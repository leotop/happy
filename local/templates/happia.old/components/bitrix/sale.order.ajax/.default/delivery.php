...
<? if(!empty($arResult["DELIVERY"])) { ?>
	<div class="bx_section onepage-checkout-step-delivery">
		...
		<h2>
			...
			<?=GetMessage("SOA_TEMPL_DELIVERY")?>
		</h2>
          <?
            CModule::IncludeModule('catalog');
            if(count($arResult["BASKET_ITEMS"]) == 1){
                $arFilter = Array("PRODUCT_ID"=>$arResult["BASKET_ITEMS"][0]['PRODUCT_ID']);
		...
		<? foreach ($arResult["DELIVERY"] as $delivery_id => $arDelivery) {
            ...
            if($arDelivery["ID"] == 14){     // вывод вкладки самовывоза в зависимости от наличия товара на складе
                if ($arStore[1]["STORE_ID"] == 4 && $arStore[1]["AMOUNT"] <= 0) {
                    $PickupMoscow = true;
               }
            } elseif($arDelivery["ID"] == 13){
               if ($arStore[0]["STORE_ID"] == 3 && $arStore[0]["AMOUNT"] <= 0){
                    $Pickup_SP = true;
               }
            }
			...
			<div class="bx_block w100 vertical <?if(($arDelivery["ID"] == 14 && $PickupMoscow) || ($arDelivery["ID"] == 13 && $Pickup_SP)){?>hide<?}?>">
							...
							<span class="bx_result_price">
								<?if(isset($arDelivery["PRICE"]))
								{
										...
										&& round($arDelivery['DELIVERY_DISCOUNT_PRICE'], 4) != round($arDelivery["PRICE"], 4))
									{
										...
										echo "</span><br/><span style='text-decoration:line-through;color:#828282;'>".(strlen($arDelivery["PRICE_FORMATED"]) > 0 ? $arDelivery["PRICE_FORMATED"] : number_format($arDelivery["PRICE"], 2, ',', ' '))."</span>";
									}
									else
									{
										echo (strlen($arDelivery["PRICE_FORMATED"]) > 0 ? $arDelivery["PRICE_FORMATED"] : number_format($arDelivery["PRICE"], 2, ',', ' '))."</span>";
									}
									echo "<br />";

									if (strlen($arDelivery["PERIOD_TEXT"])>0)
									{
										echo GetMessage('SALE_SADC_TRANSIT').": <span>".$arDelivery["PERIOD_TEXT"]."</span>";
										echo '<br />';
									}
									if ($arDelivery["PACKS_COUNT"] > 1)
									{
										echo '<br />';
										echo GetMessage('SALE_SADC_PACKS').': <span>'.$arDelivery["PACKS_COUNT"].'</span>';
									}
								}
